...
- **300 文字超の自動分割**: Bluesky の 300 grapheme 制限を超える場合、自動でスレッド化。長い URL は公式アプリと同様に省略表示
- **リッチテキスト**: URL, メンション (@handle → DID 解決), ハッシュタグを自動検出
- **リンクカード**: URL を含むポストにサムネイル付きリンクカードを自動添付 (OGP メタデータから生成)
...
- 通信先は `bsky.social` のみ (リンクカード有効時は対象 URL の OGP メタデータ・サムネイルも取得)
...
- **リプライ**: 既存ポストへの返信はデフォルトでクロスポストされない。設定で「リプライを Bluesky にポストする」を有効化すると、返信テキストも新規ポストとしてクロスポストされる
- **リンクカード**: 画像付き投稿では画像が優先される (両方の embed は共存不可)。OGP 非対応サイトはタイトルなしカードにフォールバック。設定画面で有効化時にウェブアクセス権限が付与される (無効化で自動解除)
...
- **リンクカード**: リンクカード生成時に対象サイトの OGP メタデータおよびサムネイル画像を取得し Bluesky にアップロードします。サムネイル画像の再ホスティングは各サイトの利用規約に抵触する可能性がありますが、リンクプレビュー生成として広く行われている慣行です。

本拡張の使用は自己責任で行ってください。各プラットフォームの利用規約を確認し、リスクを理解した上でご利用ください。詳細は [SECURITY_AUDIT.md](SECURITY_AUDIT.md) の「利用規約リスク分析」セクションを参照してください。
