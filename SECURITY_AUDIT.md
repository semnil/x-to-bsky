...
- 全ての fetch 呼び出しは `https://bsky.social`、`https://www.youtube.com` (oEmbed)、`https://i.ytimg.com` (サムネイル) — HTTP フォールバックなし。
...
### 6.2 YouTube oEmbed / サムネイル外部通信 (LOW)

- **場所**: `background.js:fetchYouTubeOEmbed()`, `background.js:uploadThumbnail()`
- **詳細**: YouTube URL を含むポスト投稿時に外部通信が発生する:
  1. YouTube oEmbed API (`https://www.youtube.com/oembed`) へのメタデータ取得
  2. YouTube サムネイル画像 (`https://i.ytimg.com`) のダウンロード
- **評価**:
  - 認証情報は YouTube 側に一切送信されない（oEmbed は API キー不要の公開エンドポイント）
  - oEmbed レスポンスの `title`, `author_name`, `thumbnail_url` フィールドのみ使用
  - サムネイル画像は `uploadBlob` 経由で Bluesky に送信されるのみ — ローカルに永続化されない
  - ユーザーが投稿したテキストに含まれる YouTube URL のみがトリガー（拡張が自律的に通信を行うことはない）
- **緩和策**: デフォルト有効だが設定画面から無効化可能。サムネイルのみの無効化も可能。
...
| 6.2 | YouTube oEmbed / サムネイル外部通信 | LOW | 認証情報送信なし、ユーザー操作起点 |
...
4. **Chrome Web Store 審査対策** — host_permissions に YouTube ドメインを追加した理由の説明を準備
...
- **評価**: サムネイル再アップロードは ToS の文言上は議論の余地があるが、リンクプレビューとしての利用は業界標準的な慣行。設定でサムネイルなし（タイトルのみ）に変更可能。
