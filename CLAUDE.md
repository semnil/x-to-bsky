...
├── lib.js               # 純粋関数 (grapheme処理・テキスト分割・facet解析・YouTube URL検出)
...
- `fetchYouTubeOEmbed(url)`: YouTube oEmbed API でメタデータ取得 (API キー不要)
- `uploadThumbnail(url, jwt)`: 画像 URL をダウンロードして Bluesky にアップロード
- `buildYouTubeEmbed(url, jwt)`: YouTube リンクカード (`app.bsky.embed.external`) 構築
- `createPost(text, images, parent, root)`: 画像 embed / YouTube リンクカード / reply chain 対応
...
- `extractYouTubeUrl(text)`: YouTube URL 検出 (動画・プレイリスト・チャンネル等対応)

### 認証情報の保存

- `chrome.storage.local` に `bskyHandle`, `bskyAppPassword`, `crosspostEnabled`, `customSelectors`, `includeQuoteUrl`, `includeYouTubeCard`, `youtubeCardThumbnail` を保存
- App Password を使用（メインパスワードではない）
- `host_permissions`: `bsky.social`, `*.bsky.network`, `www.youtube.com` (oEmbed API), `youtu.be`, `i.ytimg.com` (サムネイル)
...
- **YouTube リンクカード**: YouTube URL を含むポストで画像がない場合、oEmbed API でサムネイル付きリンクカードを自動生成。プレイリスト等 oEmbed 非対応 URL はタイトルなしカードにフォールバック。デフォルト有効、設定で無効化可能
...
10. YouTube URL 付きテスト投稿 → Bluesky 側でサムネイル付きリンクカードが表示されることを確認
11. YouTube プレイリスト URL → カードが生成されること (タイトルなし) を確認
12. 設定で YouTube カードを無効化 → カードが生成されないことを確認
